...
def plot_generated_images(i, img):
    # plt.figure(figsize = (,10))
    
    plt.imshow(img, interpolation='nearest')
    plt.axis('off')
    plt.savefig('fake_images_epoch_%d.png' %i)
...
def train(g_model, d_model, gan_model, latent_dim, n_epochs=1, n_batch=2048):
    ...
    for i in range(n_steps):
        ...
        print(type(X_fake), "fake_image_shape: ", X_fake.shape, "fake_image",X_fake)
        i = i+1
        # plot_generated_images(i,X_fake)
